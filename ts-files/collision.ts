...
export class CollisionHandler {
    ...
    addUnit(unit: CommandableSprite): void {
        this.units.push(unit);
    }

    private initializeEmptyGrid(): void {
        const grids: CommandableSprite[][][] = new Array<CommandableSprite[][]>(this.vSize);
        for (let y = 0; y < this.vSize; ++y) {
            grids[y] = new Array<CommandableSprite[]>(this.hSize);
            for (let x = 0; x < this.hSize; ++x) {
                grids[y][x] = [];
            }
        }
    ...
    private addUnitToGrid(unit: CommandableSprite): void {
    ...
    private checkUnitCollision(unit: CommandableSprite, x: number, y:number): void {
    ...
    private checkUnitCollisionWithGrid(unit: CommandableSprite, targetX: number, targetY: number): void {
    ...
    private isCollide(unit: CommandableSprite, another: CommandableSprite): boolean {
